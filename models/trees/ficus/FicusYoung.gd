...
static func build(mesh: TreeMeshBase, bark: Color, foliage: Color, foliage2: Color, age: float, compact: float) -> void:
	...
	# Top dome — round and full, sits at trunk apex
	mesh._visual.add_child(mesh._create_foliage_pad(Vector3(0.0, h * 1.00, 0.0), r, 0.82, foliage, 1.2))
	if t > 0.4:
		# Side sub-pads beginning to form the bubble cluster — lerp in smoothly
		var side_t := clampf((t - 0.4) / 0.6, 0.0, 1.0)
		var side_r := r * 0.60 * side_t
		mesh._visual.add_child(mesh._create_foliage_pad(Vector3(-r * 0.65, h * 0.88, 0.06),  side_r, 0.82, foliage2, 1.1))
		mesh._visual.add_child(mesh._create_foliage_pad(Vector3( r * 0.60, h * 0.88, -0.06), side_r, 0.82, foliage,  1.1))

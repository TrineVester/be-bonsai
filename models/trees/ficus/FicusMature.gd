...
static func build(mesh: TreeMeshBase, bark: Color, foliage: Color, foliage2: Color, root_color: Color, age: float, compact: float) -> void:
	...
	# Bubble clusters at branch tips — one cluster per branch, plus apex dome
	var pad := lerpf(0.44, 0.52, t) * (1.0 - compact * 0.45)
	if age >= 12.0:
		for node in mesh._create_foliage_cluster(mesh._branch_tip(Vector3(0.0, h * 0.40, 0.0),   0.0, btilt,        blen),        pad * 0.82, 0.72, foliage,  foliage2, 1.3): mesh._visual.add_child(node)
		for node in mesh._create_foliage_cluster(mesh._branch_tip(Vector3(0.0, h * 0.40, 0.0), 120.0, btilt,        blen),        pad * 0.82, 0.72, foliage2, foliage,  1.3): mesh._visual.add_child(node)
		for node in mesh._create_foliage_cluster(mesh._branch_tip(Vector3(0.0, h * 0.40, 0.0), 240.0, btilt,        blen),        pad * 0.82, 0.70, foliage,  foliage2, 1.3): mesh._visual.add_child(node)
	if age >= 20.0:
		for node in mesh._create_foliage_cluster(mesh._branch_tip(Vector3(0.0, h * 0.60, 0.0),  60.0, btilt * 0.78, blen * 0.68), pad * 0.68, 0.72, foliage2, foliage,  1.2): mesh._visual.add_child(node)
	if age >= 21.0:
		for node in mesh._create_foliage_cluster(mesh._branch_tip(Vector3(0.0, h * 0.60, 0.0), 180.0, btilt * 0.78, blen * 0.68), pad * 0.68, 0.70, foliage,  foliage2, 1.2): mesh._visual.add_child(node)
	if age >= 36.0:
		for node in mesh._create_foliage_cluster(mesh._branch_tip(Vector3(0.0, h * 0.60, 0.0), 300.0, btilt * 0.78, blen * 0.62), pad * 0.62, 0.72, foliage2, foliage,  1.2): mesh._visual.add_child(node)
	for node in mesh._create_foliage_cluster(Vector3(0.0, h * 0.92, 0.0), pad, 0.72, foliage, foliage2, 1.4): mesh._visual.add_child(node)
